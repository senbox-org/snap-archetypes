...
class ComplexOperatorParametersPanel extends JPanel {
    ...
    private final ComplexOperatorForm samForm;
    ...
    private ComplexOperatorSpectrumFormModel formModel;
    ...
    public ComplexOperatorParametersPanel(ComplexOperatorForm samForm, AppContext appContext, ComplexOperatorFormModel samModel) {
        this.appContext = appContext;
        this.samModel = samModel;
        this.samForm = samForm;
        bindingCtx = new BindingContext(samModel.getPropertySet());
        this.formModel = new ComplexOperatorSpectrumFormModel(this.appContext, samForm);
    ...
    ComplexOperatorParametersPanel(AppContext appContext, ComplexOperatorFormModel samModel, Callable<Product> productAccessor, ComplexOperatorThresholdPanel thresholdPanel) {
        ...
        this.formModel = new ComplexOperatorSpectrumFormModel(this.appContext, productAccessor);
        this.thresholdPanel = thresholdPanel;
        this.sourceProductAccessor = productAccessor;
    }

    ComplexOperatorSpectrumFormModel getFormModel() {
    ...
    private void initResampleValues() {
        ValueSet resampleTypeSet = bindingCtx.getPropertySet().getProperty(ComplexOperatorFormModel.RESAMPLE_TYPE_PROPERTY).getDescriptor().getValueSet();
        resampleTypeValues = new String[resampleTypeSet.getItems().length];
        ValueSet upsamplingSet = bindingCtx.getPropertySet().getProperty(ComplexOperatorFormModel.UPSAMPLING_PROPERTY).getDescriptor().getValueSet();
        upsamplingMethodValues = new String[upsamplingSet.getItems().length];
        ValueSet downsamplingSet = bindingCtx.getPropertySet().getProperty(ComplexOperatorFormModel.DOWNSAMPLING_PROPERTY).getDescriptor().getValueSet();
    ...
    private void bindComponents() {
        bindingCtx.bind(ComplexOperatorFormModel.REFERENCE_BANDS_PROPERTY, sourceBandNames, true);
        bindingCtx.bind(ComplexOperatorFormModel.SPECTRA_PROPERTY, spectrumList, true);
        bindingCtx.bind(ComplexOperatorFormModel.HIDDEN_SPECTRA_PROPERTY, hiddenSpectrumList, true);
    }
    ...
    private JPanel resamplingPanel() {
        ...
        bindingCtx.bind(ComplexOperatorFormModel.RESAMPLE_TYPE_PROPERTY, resampleTypeComboBox);
        samModel.getPropertySet().setValue(ComplexOperatorFormModel.RESAMPLE_TYPE_PROPERTY, resampleTypeValues[0]);
        bindingCtx.getPropertySet().getProperty(ComplexOperatorFormModel.RESAMPLE_TYPE_PROPERTY).addPropertyChangeListener(evt -> checkResampling());
        ...
        bindingCtx.bind(ComplexOperatorFormModel.UPSAMPLING_PROPERTY, upsamplingComboBox);
        samModel.getPropertySet().setValue(ComplexOperatorFormModel.UPSAMPLING_PROPERTY, upsamplingMethodValues[0]);
        ...
        bindingCtx.bind(ComplexOperatorFormModel.DOWNSAMPLING_PROPERTY, downsampligComboBox);
        samModel.getPropertySet().setValue(ComplexOperatorFormModel.DOWNSAMPLING_PROPERTY, downsamplingMethodValues[0]);
    ...
    private void checkResampling() {
        ...
        if (!needsResampling) {
            propertySet.setValue(ComplexOperatorFormModel.RESAMPLE_TYPE_PROPERTY, RESAMPLE_NONE);
        }
        setMessage(propertySet.getValue(ComplexOperatorFormModel.RESAMPLE_TYPE_PROPERTY));
        setEnabled(ComplexOperatorFormModel.RESAMPLE_TYPE_PROPERTY, needsResampling);
        if (!needsResampling) {
            messageLabel.setForeground(Color.BLUE);
            messageLabel.setText("No resample is needed");
        }
    ...
    private void setMessage(String method) {
        switch (method) {
            case RESAMPLE_LOWEST:
                ...
                setEnabled(ComplexOperatorFormModel.DOWNSAMPLING_PROPERTY, true);
                setEnabled(ComplexOperatorFormModel.UPSAMPLING_PROPERTY, false);
                messageLabel.setForeground(Color.BLUE);
                break;
            case RESAMPLE_HIGHEST:
                messageLabel.setText(String.format(RESAMPLE_MESSAGE, "highest"));
                setEnabled(ComplexOperatorFormModel.DOWNSAMPLING_PROPERTY, false);
                setEnabled(ComplexOperatorFormModel.UPSAMPLING_PROPERTY, true);
                messageLabel.setForeground(Color.BLUE);
                break;
            case RESAMPLE_NONE:
