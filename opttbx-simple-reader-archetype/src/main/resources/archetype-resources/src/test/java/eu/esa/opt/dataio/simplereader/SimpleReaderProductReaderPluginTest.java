...
public class SimpleReaderProductReaderPluginTest {
    ...
    public void setup() {
        ...
        plugIn = new SimpleReaderProductReaderPlugin();
    }

    @Test
    public void deimosReaderIsLoaded() {
        final Iterator iterator = ProductIOPlugInManager.getInstance().getReaderPlugIns(SimpleReaderConstants.DIMAP_FORMAT_NAMES[0]);
        final ProductReaderPlugIn plugIn = (ProductReaderPlugIn) iterator.next();
        assertEquals(SimpleReaderProductReaderPlugin.class, plugIn.getClass());
    }
